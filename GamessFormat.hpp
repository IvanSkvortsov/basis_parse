#ifndef __GAMESS_FORMAT_HPP__
#define __GAMESS_FORMAT_HPP__
#include"Format.hpp"
...
template<class T>
struct GamessFormat: public Format<T, gamess_regex>
{
	GamessFormat():Format<T, gamess_regex>(){}
	GamessFormat(char const * __file_name):Format<T, gamess_regex>(__file_name){}
...
template<class T>
int GamessFormat<T>::find_head(std::istream & inp)
{
	std::string & str = this->line;
	gamess_regex * regx = &this->fmt_regex;
...
template<class T>
int GamessFormat<T>::get_elementName(std::istream & inp, std::vector<std::string> & elementLabel )
{
	std::string & str = this->line;
	gamess_regex * regx = &this->fmt_regex;
	while( std::getline(inp, str) )
	{
		...
		std::cerr << "Error: [GamessFormat<T>::get_elementName(istream &, vector<string> &)]" << std::endl;
...
template<class T>
int GamessFormat<T>::get_LN(int & L, int & N)const
{
	boost::smatch result;
	gamess_regex const * regx = &this->fmt_regex;
...
template<class T>
int GamessFormat<T>::get_elementContent(std::istream & inp, std::vector<std::vector<std::vector<std::pair<T, T> > > > & elementContent)
{
